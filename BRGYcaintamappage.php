...
// Get the user's barangay from session
$userBarangay = isset($_SESSION['BrgyName']) ? $_SESSION['BrgyName'] : '';
?>
...
<!-- 
  ...
  <style>
    ...
    .windy-modal.show {
        display: flex !important; /* This ensures the modal will be shown when the 'show' class is added */
    }
    ...
    .openwindy {
        ...
        top: 530px; /* Distance from the bottom of the page */
        right: 50px; /* Distance from the right edge of the page */
        z-index: 500; /* Ensures the button stays on top */
    }
  ...
  <div class="content">
    ...
    <div id="map"></div>
      ...
      <div>
        ...
        <ul style="list-style-type: none; padding-left: 10px; margin: 5px 0;">
          <li>
            <span>Fast</span>
            <span style="color: #22c55e;">▬▬</span> <!-- green-500 -->
            <span style="color: #f59e0b;">▬▬</span> <!-- yellow-500 -->
            <span style="color: #ef4444;">▬▬</span> <!-- red-500 -->
            <span style="color: #b91c1c;">▬▬</span> <!-- red-700 -->
            <span>Slow</span>
          </li>
    ...
    <div id="detailedModal" class="detailed-modal">
      <div class="detailed-modal-content">
        ...
        <div class="openwindy">
        <!-- Button to open Windy map modal -->
        <button onclick="openWindyModal()" class="weather-button">
            View Weather Heatmap
        </button>
    </div>
    ...
    <!-- Windy Modal -->
    <div id="windyModal" class="windy-modal">
        <div class="modal-content">
            <!-- Close Button -->
            <button onclick="closeWindyModal()" class="close-btn">
                ✕
            </button>
            <!-- Windy Iframe Embed -->
            <iframe
                id="windyIframe"
                src="https://embed.windy.com/embed2.html?lat=14.5786&lon=121.1226&detailLat=14.5786&detailLon=121.1226&zoom=12&level=surface&overlay=temp&menu=&message=&marker=&calendar=&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=default"
                width="100%"
                height="100%"
                frameborder="0"
                class="iframe-style">
...
<script>
    ...
    // Get user barangay from PHP
    var userBarangay = "<?php echo $userBarangay; ?>";
    ...
    // Add OpenStreetMap layer
        ...
        // Create a base maps object to hold the layers
        var baseMaps = {
        "OpenStreetMap": openStreetMap,
        "Google Terrain": googleTerrain
        };

        // Create an overlay maps object to hold the traffic layer
        var overlayMaps = {
        ...
        // Find the user's barangay data
        var userBarangayData = barangays.find(b => b.name === userBarangay);
        
        // If user's barangay is found, set the view and add marker
        if (userBarangayData) {
            // Set the map view to the user's barangay
            map.setView(userBarangayData.coords, 15);
            
            // Create variables for map elements
            var barangayCircles = {};
            var barangayData = {};
            var roadLayers = {};
            var currentRoadLayers = [];
            
            // Add marker for user's barangay
            var marker = L.marker(userBarangayData.coords).addTo(map).bindPopup(`<span class="popup-text">${userBarangayData.name}</span>`);
            
            // Add circle for user's barangay
            barangayCircles[userBarangayData.name] = L.circle(userBarangayData.coords, {
            radius: userBarangayData.radius,
            color: "#2B3467",
            fillColor: "#2B346",
            fillOpacity: 0.1
            }).addTo(map);
            ...
            openModal(userBarangayData.name);
            });
            
            // Define roads for each barangay
            var barangayRoads = {
            "San Andres": [
                { 
                name: "Emerald Avenue", 
            ...
            // Only show evacuation centers that are close to the user's barangay
            evacuationCenters.forEach(center => {
            // Calculate distance between evacuation center and barangay (simplified)
            var distance = Math.sqrt(
                Math.pow(center.coords[0] - userBarangayData.coords[0], 2) + 
                Math.pow(center.coords[1] - userBarangayData.coords[1], 2)
            );
            
            // If center is close to user's barangay (arbitrary threshold)
            if (distance < 0.015) { // approximately 1.5km in coordinates
            ...
            if (data.barangay) {
                ...
                // Only process data for user's barangay
                if (barangayName === userBarangay) {
            ...
            let dateStr = currentDate.toLocaleString('en-US', {
                weekday: 'long',
                month: 'long',
                day: 'numeric',
                year: 'numeric',
            ...
            if (barangayData[barangayName]) {
                ...
                document.getElementById("floodStatusText").textContent = getStatusText(waterLevel);
                document.getElementById("floodStatusDot").style.backgroundColor = getStatusColor(waterLevel);
                document.getElementById("floodStatusDot").style.display = "block";
                document.getElementById("waterLevelDescription").textContent = getWaterLevelDescription(waterLevel);
            ...
            window.showPassableRoads = function() {
            let barangayName = window.selectedBarangay;
            ...
            // Fallback if user's barangay is not found
            console.error("User's barangay not found in the data");
            map.setView([14.5786, 121.1226], 14); // Default view
            ...
            errorDiv.innerHTML = '<strong>Barangay information not found</strong><br>Please contact your administrator.';
            
            document.getElementById('map').appendChild(errorDiv);
        }

    
    // Open the Windy modal
        function openWindyModal() {
            document.getElementById("windyModal").classList.add("show");
            document.body.classList.add("overflow-hidden"); // Prevent background scrolling
        }

        // Close the Windy modal
        function closeWindyModal() {
            document.getElementById("windyModal").classList.remove("show");
